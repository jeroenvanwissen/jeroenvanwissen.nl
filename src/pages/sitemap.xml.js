...
export async function get({ params, request }) {
    ...
    const collections = [];
    const categories = [];
    urlset.push({lastmod: posts[0].data.date.toISOString(), priority: '1.00', loc: import.meta.env.SITE});
    posts.forEach(post => {
        if (!collections.includes(post.collection)) {
            urlset.push({lastmod: posts[0].data.date.toISOString(), priority: '1.00', loc: `${import.meta.env.SITE}/${post.collection}`});
            collections.push(post.collection);
        }
        urlset.push({lastmod: post.data.date.toISOString(), priority: '0.80', loc: `${import.meta.env.SITE}/blog/${post.slug}`});
        generateCategories(post.data.categories).forEach(category => {
            if (!categories.includes(category.slug)) {
                urlset.push({lastmod: post.data.date.toISOString(), priority: '0.64', loc: `${import.meta.env.SITE}/category/${category.slug}`});
                categories.push(category.slug);
            }
        });
    });

    const sitemap = urlset.map(url => `<url><loc>${url.loc}</loc><lastmod>${url.lastmod}</lastmod><priority>${url.priority}</priority></url>`).join('');
